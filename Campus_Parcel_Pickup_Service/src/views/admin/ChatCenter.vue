...
const initWebSocket = () => {
  ...
  stompClient = new Client({
    // 直接使用 WebSocket 连接 SockJS 端点的 websocket 通道
...
const getSessions = async () => {
  try {
    ...
    const response = await request.get('/api/admin/chats/sessions')
...
const loadAllHistoryMessages = async () => {
  ...
  while (messagePagination.hasMore && loadAttempts < maxLoadAttempts) {
    ...
    try {
      const response = await request.get(`/api/admin/chats/sessions/${currentSessionId.value}/messages`, {
      ...
      if (response.data.code === 200) {
        const newMessages = response.data.data?.records || response.data.data || []
...
const markAsRead = async () => {
  ...
  try {
    await request.post(`/api/admin/chats/sessions/${currentSessionId.value}/read`)
...
const handleSearch = async () => {
  ...
  try {
    const response = await request.get('/api/admin/chats/search', {
